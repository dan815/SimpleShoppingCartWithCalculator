...
public class PriceCalculatorTest {
	...
	public void init() {
	  ...
	  @Test public void test_price_calculation_of_empty_basket() { Basket basket =
	  new Basket(); assertEquals(PriceCalculator.getBasketPrice(basket),0.0,0.002);
	  }
	 
	 
	@Test
	public void test_price_calculation_of_basket__with_items_with_cheapest_being_free() {
		// only one free
		assertEquals(PriceCalculator.getBasketPrice(basket), 11.22, 0.002);
	}
	 ...
	 * 
	 * @Test public void
	 * test_price_calculation_of_basket__with_items_with_very_large_prices() { Item
	 * orange = new Item(5,ItemTypeEnum.ORANGE,Integer.MAX_VALUE); Item lemon = new
	 * Item(6,ItemTypeEnum.LEMON,Integer.MAX_VALUE);
	 * 
	 * List<Item> itemList = Arrays.asList(orange,lemon); basket = new
	 * Basket(itemList); //should throw custom exception
